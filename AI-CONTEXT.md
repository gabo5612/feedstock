...
> **crucible — local-first input-cost risk for a metals plant**
>
...
2. `src/crucible/instruments.py` — the registry and why `ZN=F` is not zinc
3. `src/crucible/sources/yahoo.py` — why `range=max` lies and you have to paginate
4. `src/crucible/leakcheck.py` — the leakage guard: the artefact that matters most
5. `src/crucible/signals.py` — the buy signal and its backtest against buying blind
6. `src/crucible/basket.py` — the cost basket, exposure and scenarios
7. `src/crucible/quality.py` — the data-quality monitor and why it corrected its own message
